...
const SEZIONE_SLUG = "dati-generali";
              ...
              {dati.map((rilevazione) => (
                <TableRow key={rilevazione.id}>
                  ...
                  <TableCell className="flex justify-end gap-1">
                    <PinToggleButton
                      pinned={isRecordPinned(SEZIONE_SLUG, rilevazione.id)}
                      onToggle={() => togglePinRecord(SEZIONE_SLUG, rilevazione.id, `Dati generali ${rilevazione.anno_riferimento}`)}
                    />
                    {inModifica && (
                      <>
                        <DatiGeneraliDialog
