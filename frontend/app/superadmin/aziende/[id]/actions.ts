...
export async function aggiornaAbbonamento(
  ...
  try {
    await apiFetch<Abbonamento>(`/api/superadmin/aziende/${aziendaId}/abbonamenti/${certificazioneId}`, {
      method: "PUT",
  ...
  revalidatePath(`/superadmin/aziende/${aziendaId}`);
  return { success: true };
}
...
export async function disattivaAbbonamento(
  ...
  await apiFetch<Abbonamento>(`/api/superadmin/aziende/${aziendaId}/abbonamenti/${certificazioneId}/disattiva`, {
    method: "POST",
  });
  revalidatePath(`/superadmin/aziende/${aziendaId}`);
}
