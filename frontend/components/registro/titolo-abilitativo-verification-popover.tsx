...
import { inviaVerificaTitoloAbilitativo } from "@/app/(app)/anagrafica/titoli-abilitativi/actions";
import { formatDateTime } from "@/lib/format";
import type { TitoloAbilitativoSummary } from "@/lib/types/anagrafica";

/** Popup di verifica ancorato alla riga di un titolo abilitativo (tabella
 * unificata "Albi, ruoli, licenze e certificazioni", § Correzione 20 punto
 * 4 "Stato"): stesso trattamento — nota sempre presente, azioni coerenti
 * con lo stato corrente, verificato da/il — di `IncaricoVerificationPopover`
 * (Soci/Amministratori/Sindaci), mai un secondo sistema di verifica
 * (§ app/core/verifica_riga.py). */
export function TitoloAbilitativoVerificationPopover({
  titolo,
  consulente,
  onDecided,
  disabled = false,
}: {
  titolo: TitoloAbilitativoSummary;
  consulente: boolean;
  onDecided: () => void;
  ...
  const status = titolo.verificationStatus ?? "PENDING_VERIFICATION";
  ...
  const nomeRiga = titolo.tipologia_label;

  function apri(v: boolean) {
    setOpen(v);
    if (v) setNota(titolo.revisionNote ?? "");
    else setErrore(null);
  }
  ...
  async function decidi(decision: "VERIFIED" | "REVISION_REQUIRED") {
    ...
    setInvio(true);
    setErrore(null);
    const esito = await inviaVerificaTitoloAbilitativo(
      titolo.id,
      decision,
      nota.trim() || null,
      titolo.verificationVersion,
    );
    setInvio(false);
    if (esito.esito === "ok") {
      setOpen(false);
      ...
      >
        ...
        {isVerified && (
          <div className="mb-4 grid gap-[13px] rounded-lg border border-[#ccebdc] bg-[#f2fbf7] p-[13px]">
            ...
            {(titolo.verifiedAt || titolo.verifiedBy) && (
              <p className="text-[11px] leading-snug text-[#526a8f]">
                Verificato il <strong className="font-bold text-[#263a68]">{formatDateTime(titolo.verifiedAt)}</strong>
                {titolo.verifiedBy && (
                  <>
                    {" "}
                    da <strong className="font-bold text-[#263a68]">{titolo.verifiedBy}</strong>
                  </>
                )}
              </p>
            )}
        ...
        {consulente ? (
          ...
          <div className="grid gap-2">
            ...
            <p className="min-h-[42px] rounded-[7px] border border-[#e6ebf3] bg-[#f8fafc] p-[13px] text-xs leading-snug text-[var(--az-ink)]">
              {titolo.revisionNote?.trim() ? titolo.revisionNote : "Nessuna nota."}
            </p>
          </div>
        )}
